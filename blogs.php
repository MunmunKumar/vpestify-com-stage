...
include_once("admin-panel/include/config.php");
    ...
    $selQry = $conn->query("select * from blog where IsDelete = '0'");
...
<div class="service-individual-container">
    <div class="container">    
        ...
        <div class="col-md-12 text-center font-italic"><h2 class="font-size-50"><u>Blogs</u></h2></div>

                <?php

                 $sid = 0;
                    if($selQry->num_rows>0){
                        //while ($row = $selQry->fetch_assoc()) {
                            foreach($selQry as $row){
                            $sid++;
                        ?>
                            <div class="col-lg-4 col-md-6 col-sm-6">
                                <div class="single-blog-container img-container" onclick="window.location.href = 'blog-posts?blog=<?php echo $row['BlogUrl']?>'">
                                    <img alt="bird-control-net-services" src="admin-panel/gallery-image/blogs/<?php echo $row['BlogImage']?>" class="img-responsive width-348 height-260">
                                    <div class="text-center mt-3">
                                        <h6><i class="fa fa-calendar" aria-hidden="true"></i> 
                                            <?php echo date('F d, Y', strtotime($row['CreatedOn']));  ?> &nbsp;&nbsp;&nbsp;
                                            <?php echo $row['PostBy']; ?>
                                        </h6>
                                    </div> 
                                    
                                    <div class="text-justify mt-2 padding-15">
                                        <?php echo  substr($row['ShortDesc'],0, 200) ?>
                                    </div> 

                                    <div class="text-center padding-10">
                                        <Button type="button" class="btn btn-red">Read More</Button>
                                    </div>
                                </div>
                            </div>
